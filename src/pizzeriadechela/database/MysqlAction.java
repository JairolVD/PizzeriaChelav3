...
public class MysqlAction extends MysqlBaseDatos {
    ...
    public Boolean addPedido(PedidosDto dto){
        try{
            ...
            this.crearConsulta(
                    "INSERT INTO pedidos (DNI_C, DNI_V, DNI_R, PizzaDetalleId) VALUES" + 
                    "('" + dto.getDniC()+ "'," +
                    "'"  + dto.getDniV() + "'," +
                    "'"  + dto.getDniR() + "'," +
    ...
    public Boolean updatePedidoDetalle(PizzaDetalleDto dto){
        try{
            this.crearConsulta(
                    "UPDATE pizza_detalle SET " + 
                    "MetodoPago='" + dto.getMetodoPago()+ "'," +
                    "Tamanio='" + dto.getTamanio()+ "'," +
                    "Cantidad='" + dto.getCantidad()+ "'," +
                    "Estado='" + dto.getEstado()+ "' WHERE (Id= " + dto.getId()+ ")"
                ).executar();
            this.closeConsulta();
            return true;
        }catch(SQLException e){
    ...
    public PedidosDto getRepartidor(String dni_r){
        try{
            ...
            ResultSet resultado = this.crearConsulta(
                    "SELECT * FROM pedidos a JOIN pizza_detalle b ON a.PizzaDetalleId=b.Id JOIN comprobante c ON a.Id=c.PedidoId "
                            + "JOIN pizzas d ON b.PizzaId=d.Id WHERE (DNI_R= '" + dni_r + "') ORDER BY b.Id DESC").getResultado();
            ...
            pedido.setPedidoDetalleId(resultado.getInt(2));
            pedido.setDniC(resultado.getString(3));
            pedido.setDniV(resultado.getString(4));
            pedido.setDniR(resultado.getString(5));
            pedido.getDetalle().setId(resultado.getInt(6));
            pedido.getDetalle().setPizzaId(resultado.getInt(7));
            pedido.getDetalle().setMetodoPago(resultado.getByte(8));
            pedido.getDetalle().setTamanio(resultado.getString(9));
            pedido.getDetalle().setCantidad(resultado.getByte(10));
            pedido.getDetalle().setEstado(resultado.getByte(11));
            pedido.getComprobante().setId(resultado.getInt(12));
            pedido.getComprobante().setPedidoId(resultado.getInt(13));
            pedido.getComprobante().setFecha(resultado.getString(14));
            pedido.getDetalle().setPizzas(new PizzasDto());
            pedido.getDetalle().getPizzas().setId(resultado.getInt(15));
            pedido.getDetalle().getPizzas().setNombre(resultado.getString(16));
            pedido.getDetalle().getPizzas().setDescripcion(resultado.getString(17));
            pedido.getDetalle().getPizzas().setOferta(resultado.getByte(18));
            pedido.getDetalle().getPizzas().setPrecio(resultado.getDouble(19));
            pedido.getDetalle().getPizzas().setImagenUrl(resultado.getString(20));
            this.closeConsulta();
            return pedido;
        }catch(SQLException e){
            e.printStackTrace();
    ...
    public PedidosDto getPedido(String dni_c){
        try{
            PedidosDto pedido = new PedidosDto();
            pedido.setDetalle(new PizzaDetalleDto());
            pedido.setComprobante(new ComprobanteDto());
            ResultSet resultado = this.crearConsulta(
                    "SELECT * FROM pedidos a JOIN pizza_detalle b ON a.PizzaDetalleId=b.Id JOIN comprobante c ON a.Id=c.PedidoId "
                            + "JOIN pizzas d ON b.PizzaId=d.Id WHERE (DNI_C= '" + dni_c + "')").getResultado();
    ...
    public ArrayList<CuentaDto> getTrabajadores(){
        try{
            ...
            ResultSet resultado = this.crearConsulta(
                    "SELECT * FROM cuentas a JOIN cuenta_perfil b ON a.DNI=b.DNI_C WHERE (a.Cargo=1 OR a.Cargo=2)"
                ).getResultado();
    ...
    public ArrayList<CuentaDto> getRepartidores(){
        try{
            ArrayList<CuentaDto> trabajadores = new ArrayList<CuentaDto>();
            ResultSet resultado = this.crearConsulta(
                    "SELECT * FROM cuentas a JOIN cuenta_perfil b ON a.DNI=b.DNI_C WHERE (a.Cargo=2)"
                ).getResultado();
    ...
    public ArrayList<PedidosDto> getPedidos(String dni_c){
        try{
